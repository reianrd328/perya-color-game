...
# Connect to Aiven MySQL using PyMySQL securely
...
# Admin login — set these in your host environment, never in source.
...
def handle_deny_roll(data):
    ...
    if target_sid:
        socketio.emit('roll_denied', {'message': "The admin denied your roll request."}, to=target_sid)
...
def handle_deny_withdraw(data):
    ...
    target_sid = online_players.get(username)
    if target_sid:
        socketio.emit('withdraw_status', {'message': "🚫 The admin denied your withdrawal request."}, to=target_sid)
